...
Firebase RTDB (income-41a40 專案,與收入 app 共用,節點隔離在 paperRadar/)
  ├─ feed              ← 論文清單(唯讀,後端寫)
  └─ userState/{uid}   ← 已讀/vote/flag/star(前端讀寫;每筆含 doi)+ downloaded(下載器回寫,前端顯示 ✓ 已收錄)
        │
        ▼
...
- **去重 + 跨裝置狀態**:成功後回寫 Firebase `userState.downloaded=true`(admin `update` 只動這欄,不覆蓋 star/vote/flag);既是去重依據(LLM wiki 把 `new/`→`raw/` 封存後也不會被重抓),**前端也據此把 ⭐ 顯示成「✓ 已收錄」**,跨裝置看得到。非 OA 的不標 → 下次會重試(哪天開放 OA 就抓到)。`--redo` 強制重抓。
